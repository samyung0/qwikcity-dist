  ...
  "node_modules/.pnpm/yaml@2.3.2/node_modules/yaml/dist/doc/createNode.js"(exports) {
    ...
    function createNode(value2, tagName, ctx) {
      ...
      if (identity.isPair(value2)) {
        const map2 = (_b2 = (_a2 = ctx.schema[identity.MAP]).createNode) == null ? void 0 : _b2.call(_a2, ctx.schema, null, ctx);
...
var require_resolve_flow_scalar = __commonJS({
  "node_modules/.pnpm/yaml@2.3.2/node_modules/yaml/dist/compose/resolve-flow-scalar.js"(exports) {
    ...
    function resolveFlowScalar(scalar, strict, onError) {
      ...
      let _type;
      let value2;
      const _onError2 = (rel, code2, msg) => onError(offset + rel, code2, msg);
      switch (type) {
        case "scalar":
...
var require_cst_scalar = __commonJS({
  "node_modules/.pnpm/yaml@2.3.2/node_modules/yaml/dist/parse/cst-scalar.js"(exports) {
    ...
    function resolveAsScalar(token, strict = true, onError) {
      if (token) {
        const _onError2 = (pos, code2, message) => {
...
function tokenizeProtocolAutolink(effects, ok3, nok) {
  ...
  function protocolPrefixInside(code2) {
    ...
    if (code2 === 58) {
      const protocol2 = buffer.toLowerCase();
...
var wwwPrefix, domain, path, trail, emailDomainDotTrail, wwwAutolink, protocolAutolink, emailAutolink, text, gfmAutolinkLiteral, code;
...
var CONTINUE2, EXIT2, SKIP2, visitParents2;
var init_lib4 = __esm({
  "node_modules/.pnpm/unist-util-visit-parents@5.1.3/node_modules/unist-util-visit-parents/lib/index.js"() {
    init_unist_util_is();
    init_color();
...
var constructsWithoutStrikethrough, gfmStrikethroughFromMarkdown, gfmStrikethroughToMarkdown;
var init_lib8 = __esm({
  "node_modules/.pnpm/mdast-util-gfm-strikethrough@1.0.3/node_modules/mdast-util-gfm-strikethrough/lib/index.js"() {
...
function inlineCode(node, _2, state) {
...
function hasProperty2(node, field) {
  const value2 = typeof field === "string" && isNode(node) && node.type === "element" && node.properties && own9.call(node.properties, field) && node.properties[field];
  return value2 !== null && value2 !== void 0 && value2 !== false;
}
function isNode(value2) {
  return Boolean(value2 && typeof value2 === "object" && "type" in value2);
}
var own9;
...
// node_modules/.pnpm/hast-util-has-property@2.0.1/node_modules/hast-util-has-property/index.js
...
function rehypeAutolinkHeadings(options2 = {}) {
  ...
  function around(node, index, parent) {
    ...
    parent.children.splice(index, 1, ...nodes);
    return [SKIP2, index + nodes.length];
  }
  function wrap(node) {
...
import { basename as basename4, join as join6, resolve as resolve3, extname } from "path";
...
/** Qwik City Plan */`);
  ...
  c2.push(`export const allowedParams = ${JSON.stringify(ctx.opts.allowedParams)};`);
...
function normalizeOptions(rootDir, viteBasePath, userOpts) {
  ...
  if (typeof opts.allowedParams !== "object") {
    opts.allowedParams = {};
  }
  opts.mdx = opts.mdx || {};
...
async function validatePlugin(opts) {
  ...
  try {
    const s2 = await fs.promises.stat(opts.routesDir);
    if (!s2.isDirectory()) {
...
parseMarkdown_fn = function(lexer2, parser2) {
  return (src, opt, callback) => {
    ...
    if (callback) {
      ...
      this.walkTokens(tokens, (token) => {
        if (token.type === "code") {
          pending++;
          setTimeout(() => {
            highlight2(token.text, token.lang, (err, code2) => {
              if (err) {
                return done(err);
              }
    ...
    if (opt.async) {
      return Promise.resolve(opt.hooks ? opt.hooks.preprocess(src) : src).then((src2) => lexer2(src2, opt)).then((tokens) => opt.walkTokens ? Promise.all(this.walkTokens(tokens, opt.walkTokens)).then(() => tokens) : tokens).then((tokens) => parser2(tokens, opt)).then((html3) => opt.hooks ? opt.hooks.postprocess(html3) : html3).catch(throwError);
    }
    try {
      if (opt.hooks) {
        src = opt.hooks.preprocess(src);
...
function minimalHtmlResponse(status, message) {
  ...
  const color3 = status >= 500 ? COLOR_500 : COLOR_400;
  return `
<head>
  <meta charset="utf-8">
  ...
  <style>
    body { color: ${color3}; background-color: #fafafa; padding: 30px; font-family: ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, Roboto, sans-serif; }
    p { max-width: ${width}; margin: 60px auto 30px auto; background: white; border-radius: 4px; box-shadow: 0px 0px 50px -20px ${color3}; overflow: hidden; }
    strong { display: inline-block; padding: 15px; background: ${color3}; color: white; }
...
function getQwikCityServerData(requestEv) {
  ...
  const protocol2 = headers.get("X-Forwarded-Proto");
  if (host) {
    reconstructedUrl.port = "";
    reconstructedUrl.host = host;
  }
  if (protocol2) {
    reconstructedUrl.protocol = protocol2;
  }
  return {
...
function matchRoute(route, path3, allowedParams) {
  ...
  return matchRoutePart(route, routeIdx, routeLength, path3, pathIdx, pathLength, allowedParams);
}
function matchRoutePart(route, routeIdx, routeLength, path3, pathIdx, pathLength, allowedParams) {
  let params = null;
  while (routeIdx < routeLength) {
    ...
    if (routeCh === 91 /* OPEN_BRACKET */) {
      ...
      const isOptional = route.charCodeAt(paramNameEnd - 1) === 46 /* DOT */;
...
function recursiveScan(paramName, suffix, path3, pathStart, pathLength, route, routeStart, routeLength, allowedParams) {
  ...
  while (pathIdx >= pathStart && depthWatchdog--) {
    ...
    pathIdx = lastIndexOf(path3, pathStart, sep, pathIdx, pathStart - 1) + sep.length;
  }
  return null;
}
function lastIndexOf(text2, start, match, searchIdx, notFoundIdx) {
  let idx = text2.lastIndexOf(match, searchIdx);
  if (idx == searchIdx - match.length) {
    idx = text2.lastIndexOf(match, searchIdx - match.length - 1);
  }
...
async function postBuild(clientOutDir, basePathname, userStaticPaths, format, cleanStatic) {
  ...
  const loadItem = async (fsDir, fsName, pathname) => {
    ...
    if (fsName === "index.html" || fsName === "q-data.json") {
      if (!staticPaths.has(pathname) && cleanStatic) {
        await fs6.promises.unlink(fsPath);
      }
      return;
    }
    if (fsName === "404.html") {
      const notFoundHtml = await fs6.promises.readFile(fsPath, "utf-8");
      notFounds.push([pathname, notFoundHtml]);
      return;
    }
    const stat = await fs6.promises.stat(fsPath);
    if (stat.isDirectory()) {
  ...
  const loadDir = async (fsDir, pathname) => {
    const itemNames = await fs6.promises.readdir(fsDir);
      ...
      load: {
        ...
        handler: async (id) => {
          ...
          if (extension === ".svg" && params.has("jsx")) {
            const code2 = await fs7.promises.readFile(pathId, "utf-8");
            return {
              code: code2,
