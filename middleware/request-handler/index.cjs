...
var loadRoute = async (routes, menus, cacheModules, pathname, allowedParams) => {
...
async function loadRequestHandlers(qwikCityPlan, pathname, method, checkOrigin, renderFn, allowedParams) {
  const { routes, serverPlugins, menus, cacheModules } = qwikCityPlan;
  const route = await loadRoute(routes, menus, cacheModules, pathname, allowedParams);
